    ...
    <div class="root">
        <header class="tools">
            <div class="tools__left">
                ...
                <!-- <span @click="onMark('delete')" :class="[
                    'tools__left--item',
                    isMarking === 'delete' && 'tools__left--actived'
                ]">
                    <DeleteOutlined /> -->
                <!-- <span>测距</span> -->
                <!-- </span> -->
...
let holeNum: number = null; // 管孔序号为了保证唯一使用时间戳
...
let rangingP1: THREE.Vector3 = null; // 测距起点坐标
let rangingP2: THREE.Vector3 = null; // 测距终点坐标
let rangingClickNum: number = 0; // 测距点击次数
let rangingNum: number = null; // 测距序号为了保证唯一使用时间戳

UsePlaneDrag({ twin, sphereEndDragList, planeParamsList });
const { holeDragedList } = UseHoleDrag({ twin, holeDragList, holeNum });
...
const onDrewPlane = (event: MouseEvent) => {
    ...
    if (clickNum === 1) {
        ...
        if (p1 && p2) {
            // 画剖面
            const { length, width, depth, rect, sizeAC, sizeDB, sizeAD, sizeCB, pageNumber } =
...
const onDrewHole = (event: MouseEvent) => {
    holeNum = +new Date();
    setTimeout(() => {
        console.log('1111111111111111holeNum-----', holeNum);
        const point = rayCasterPoint(event);
...
const rangingClick = (event: MouseEvent, rangingNum: number, type: string) => {
    ...
    const sphere = createSphere(point, twin.camera);
    const name = `${type}点坐标`;
    sphere.name = `测距-${name}-${rangingNum}`;
    sphere.userData = {
        type: '测距',
        name,
        rangingNum
...
const onClick = (event: MouseEvent) => {
    ...
    if (!mesh) return;
    const { type } = mesh.object.userData;
    ...
    twin.scene.children.forEach((item) => {
        ...
        if (item.name === '测距组') {
            ...
            item.children.forEach((ele) => {
                if (ele.userData.rangingNum === mesh.object.userData.rangingNum) {
                    list.push(ele);
                }
            });
            item.remove(...list);
            ...
            console.log('item-type', item.name, item);
        }
    });
};
